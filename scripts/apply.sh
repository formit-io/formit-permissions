...
while getopts ":r:o:e:" opt; do
  case ${opt} in
    ...
    e ) # repo email
      ...
      ;;
    \? )
      echo "Usage: apply.sh [-r (repo name)] [-o (repo owner)] [-e (repo email)] [-a (aws account id)]"
      exit 1
      ;;
    : )
...
export TERRAGRUNT_DOWNLOAD="$TERRAGRUNT_WORKING_DIR/.terragrunt-cache/$repo_owner/$repo_name/$AWS_ACCOUNT_ID"

plan_file="$GITHUB_WORKSPACE/.out/$repo_owner-$repo_name-$AWS_ACCOUNT_ID.tfplan"
